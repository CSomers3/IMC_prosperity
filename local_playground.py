...
if __name__ == "__main__":
    ...
    bananas_average_profits: list[float] = []
    pearls_average_profits: list[float] = []
    list_of_potential_percent_put_when_mm: list[int] = [0, 1, 2, 5, 10, 20, 50, 100]
    for percent_put_when_mm in list_of_potential_percent_put_when_mm:
        for product in "bananas", "pearls":
            Algo.PERCENT_PUT_WHEN_MM = percent_put_when_mm
        ...
        bananas_average_profits.append(
            sum([all_profits[i]["BANANAS"] for i in range(len(all_profits))]) / len(all_profits)
        )
        pearls_average_profits.append(
            sum([all_profits[i]["PEARLS"] for i in range(len(all_profits))]) / len(all_profits)
        )

    plt.plot(list_of_potential_percent_put_when_mm, bananas_average_profits, label="BANANAS")
