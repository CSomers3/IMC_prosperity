...
if __name__ == "__main__":
    ...
    bananas_best_average_profit: tuple[str, float] = ("Test", 0)
    pearls_best_average_profit: tuple[str, float] = ("Test", 0)
    ...
    list_of_potential_ema_short_period: list[int] = [3, 5, 8, 12]
    list_of_potential_ema_long_period: list[int] = [12, 15, 20, 30]
        ...
        for ema_short_period in list_of_potential_ema_short_period:
            for ema_long_period in list_of_potential_ema_long_period:
                ...
                for day in "0":
                    ...
                    # df_simulation = df_simulation[:int(len(df_simulation) * 0.5)]
                    ...
                    for timestamp in df_simulation["timestamp"].unique():
                        ...
                        for product in products:
                            ...
                            # Find the first row of df_simulation where the product is the one we are looking for (right
                            # product and right timestamp)
                            ...
                            if product in orders:
                                ...
                                for order in orders[product]:
                                    if order.quantity != 0:
                                        ...
                                        if order.quantity > 0:
                                            # let's check in data_trades[f"trades_round_1_day_{day}"] if we find a trade
                                            # with the right timestamp and the right product and the right price (it's a
                                            # buy order here, so we want to find trades that were made at a lower price
                                            # than the one we are) If we find one, we add the quantity to our position
                                            # and update the PnL
                                            trades = data_trades[
                                            ...
                                            # let's check in data_trades[f"trades_round_1_day_{day}"] if we find a trade
                                            # with the right timestamp and the right product and the right price (it's a
                                            # sell order here, so we want to find trades that were made at a higher
                                            # price than the one we are) If we find one, we add the quantity to our
                                            # position and update the PnL
                                            trades = data_trades[
                                                f"trades_round_1_day_{day}_nn.csv"
                                            ]
                                            trades = trades[trades["symbol"] == product]
                        ...
                        # Liquidate positions at the end of the timestamp to have an accurate estimation of PnL at the
